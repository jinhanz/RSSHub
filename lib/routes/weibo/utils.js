...
const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = [], isRetweet = false) => {
        ...
        if (showAuthorInDesc && isRetweet) {
            let usernameAndAvatar = `<div style="display:flex; padding-bottom:0.5em"><a href="https://weibo.com/${status.user.id}" target="_blank">`;
            if (showAuthorAvatarInDesc) {
                usernameAndAvatar += `<img style="float:left; border-radius:50%!important; max-width:${sizeOfAuthorAvatar}!important; height:${sizeOfAuthorAvatar}!important" src="${status.user.profile_image_url}" ${
                    readable ? 'hspace="8 !important" vspace="4 !important" align="center"' : ''
                } /></a>`;
            }
            usernameAndAvatar += `<div style="opacity:0.6!important; display:inline-block!important;vertical-align:middle">`;
