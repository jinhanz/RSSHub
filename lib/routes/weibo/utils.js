...
const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = [], isRetweet = false) => {
        ...
        const mergedParams = {
            ...
            sizeOfAuthorAvatar: fallback(params.sizeOfAuthorAvatar, queryToInteger(routeParams.sizeOfAuthorAvatar), 45),
        ...
        htmlNewLineUnreplaced = htmlNewLineUnreplaced.replace(/(?<=<span class=["']?url-icon["']?>)<img/g, '<img style="height: 1em !important"');
        // 去掉全文
        ...
        if (showAuthorInDesc && isRetweet) {
            let usernameAndAvatar = `<div style="display:flex; align-items: center; padding-bottom:0.5em"><a href="https://weibo.com/${status.user.id}" target="_blank">`;
            if (showAuthorAvatarInDesc) {
                usernameAndAvatar += `<img style="border-radius:50%!important; width:${sizeOfAuthorAvatar}!important; height:${sizeOfAuthorAvatar}!important" src="${status.user.profile_image_url}" ${
                    readable ? 'hspace="8 !important" vspace="4 !important" align="center"' : ''
                } /></a>`;
            }
            usernameAndAvatar += `<div style="opacity:0.6; display:inline-block;vertical-align:middle">`;
            let name = status.user.screen_name;
            if (showAtBeforeAuthor) {
                name = '@' + name;
            }
            if (authorNameBold) {
                usernameAndAvatar += `<strong>${name}</strong>&ensp;`;
            } else {
                usernameAndAvatar += `${name}&ensp;`;
            }
        ...
        const picsCount = status.pics && status.pics.filter((pic) => pic.type !== 'livephotos').length;
        ...
        if (status.retweeted_status) {
            if (readable) {
                html += `<br clear="both" /><div style="clear: both"></div><br><div style="border-top: 3px solid gray; padding-top: 1em;">`;
            } else {
                html += `<br><div> - 转发 `;
            }
            ...
            retweeted += weiboUtils.formatExtended(ctx, status.retweeted_status, retweetedParams, picsPrefixes, (isRetweet = true)).description;
        ...
        if (!status.retweeted_status || showRetweetTextInTitle) {
            title += htmlNewLineUnreplaced
                .replace(/<span class=["']url-icon["']><img\s[^>]*?alt=["']?([^>]+?)["']?\s[^>]*?\/><\/span>/g, '') // 不显示表情文字
        ...
        // TODO: retrieve retweeted media info
        if (picsCount > 0) {
            title = '🖼*' + picsCount + ' ' + title;
        }
        if (livePhotoCount > 0) {
            title = '📸*' + livePhotoCount + ' ' + title;
        }
        if ((status.page_info && status.page_info === 'video') || (status.retweeted_status && status.retweeted_status.pageInfo && status.retweeted_status.pageInfo === 'video')) {
            title = '🎞 ' + title;
        }
