...
const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = [], isRetweet = false) => {
        ...
        htmlNewLineUnreplaced = htmlNewLineUnreplaced.replace(/(?<=<span class=["']?url-icon["']?>)<img/g, '<img style="margin-top:3" width="15px"');
        // 去掉全文
        ...
        if (showAuthorInDesc && isRetweet) {
            ...
            if (showAuthorAvatarInDesc) {
                usernameAndAvatar += `<img style="margin-top: 3;" width="${sizeOfAuthorAvatar}" height="${sizeOfAuthorAvatar}" src="${status.user.profile_image_url}" ${readable ? 'hspace="8" vspace="0" align="left"' : ''} />`;
            }
            let name = status.user.screen_name;
            usernameAndAvatar += `转发: <a href="https://weibo.com/${status.user.id}" target="_blank">`
        ...
        if (status.retweeted_status) {
            if (readable) {
                html += `<br clear="both" /><div style="clear: both"></div><br><div style="border-top: 3px solid gray;">`;
            } else {
                html += `<br><div> - 转发 `;
            }
