...
const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = [], isRetweet = false) => {
        ...
        if (showAuthorInDesc && isRetweet) {
            let usernameAndAvatar = `<p>转发: <a href="https://weibo.com/${status.user.id}" target="_blank">`;
            if (showAuthorAvatarInDesc) {
                usernameAndAvatar += `<img style="border-radius:50%" width="${sizeOfAuthorAvatar}" height="${sizeOfAuthorAvatar}" src="${status.user.profile_image_url}" ${readable ? 'hspace="8" vspace="0" align="left"' : ''} />`;
            }
