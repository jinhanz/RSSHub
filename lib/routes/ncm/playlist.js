...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.trackIds.slice(0, range).map((item) => {
            ...
            let prefix = ''
            if (id == 60198) {prefix = `[${songs.findIndex((element) => element.id === item.id)+1}] `};
            ...
            return {
                ...
                }"><iframe frameborder="no" border="0" marginwidth="0" marginheight="0" width=370 height=86 src="https://simple-notion-widgets.vercel.app/music-player/?server=netease&type=song&id=${item.id}"></iframe>`,
