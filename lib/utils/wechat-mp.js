...
// TODO: formatting -> empty lines, image max size
...
const normalizeUrl = (url, bypassHostCheck = false) => {
    ...
    if (/^\/s\/.+/.test(urlObj.pathname)) {
        ...
        const chksm = urlObj.searchParams.get('chksm'); // Shaderein: should keep chksm for downstream parsing
        if (biz && mid && idx && sn) {
...
const fetchArticle = (ctx, url, bypassHostCheck = false) => {
    ...
    return ctx.cache.tryGet(url, async () => {
        ...
        // Add cover images
        description = `<img width="0" height="0" hidden="true" src="${$('meta[property=og:image]').attr('content')}" referrerpolicy="no-referrer">` + description;
