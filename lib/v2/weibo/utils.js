...
const weiboUtils = {
    ...
    formatExtended: (ctx, status, uid = undefined, params = {}, picsPrefixes = [], isRetweet = false) => {
        ...
        if (status.pics && (status.pics.length != status.pic_num) && !(status.pic_num==1 && status.page_info.type=='video')) {
            // Shaderein: can't fetch additional images in reposted mblog
            html = `<p><small>请前往原微博查看完整共${status.pic_num}张图片</small></p>` + html;    
        }
        ...
        if (status.retweeted_status && status.retweeted_status.pics) {
            picsCount += status.retweeted_status.pic_num;
        }
        if (status.pics) { picsCount += status.pic_num; }
        if (picsCount > 1) {
    ...
    formatVideo: (itemDesc, status) => {
        ...
        if (pageInfo && pageInfo.type === 'video') {
            ...
            const relay = config.hotlink.relay ? config.hotlink.relay + `/weibo/video?uid=${status.user.id}&bid=${status.bid}&br=BR` : null;
            ...
            if (hasVideo) {
                ...
                if (video720p) {
                    video += `<source src="${relay ? relay.replace('BR','720p') : video720p}">`;
                }
                if (videoHd) {
                    video += `<source src="${relay ? relay.replace('BR','Hd') : videoHd}">`;
                }
                if (videoHdHevc) {
                    video += `<source src="${relay ? relay.replace('BR','HdHevc') : videoHdHevc}">`;
                }
                if (videoLd) {
                    video += `<source src="${relay ? relay.replace('BR','Ld') : videoLd}">`;
                }
                // if (pageUrl) {
