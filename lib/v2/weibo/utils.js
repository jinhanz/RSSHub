    ...
    formatExtended: (ctx, status, uid = undefined, params = {}, picsPrefixes = [], isRetweet = false) => {
        ...
        if (showAuthorInDesc && isRetweet) {
            ...
            if (showAuthorAvatarInDesc) {
                usernameAndAvatar += `<img style="margin-top: 3;" width="${sizeOfAuthorAvatar}" max-width="${sizeOfAuthorAvatar}" height="${sizeOfAuthorAvatar}" max-height="${sizeOfAuthorAvatar}" src="${status.user.profile_image_url}" ${readable ? 'hspace="8" vspace="0" align="left"' : ''} /> `;
            }
        ...
        if (pics && pics.length>0) {
            ...
            let picsPrefix = '';
            picsPrefix += `<img width="0" height="0" hidden src="${pics[0].large.url.replace('/large/','/mw2000/')}">`;
