    ...
    let resultItems = await Promise.all(
        cards
            ...
            .map(async (item) => {
                ...
                link = `https://m.weibo.cn/status/${item.mblog.bid}`;
                // If choose to use retweeted status url in feed, put original url at the bottom
                if (retweet && useRepostedUrl === '1') {
                    description = description + `<br><br><a href="${link}">转发此条的微博</a>`;
                    link = `https://m.weibo.cn/status/${item.mblog.retweeted_status.bid}`;
                }

                description = description.replaceAll(/wx(?=[1-4])/g, 'tva');
