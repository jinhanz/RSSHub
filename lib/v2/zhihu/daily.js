    ...
    const items = await Promise.all(
        articleList.map(async (item) => {
            ...
            const description = await ctx.cache.tryGet(item.link, async () => {
                ...
                const cover = storyDetail.data.image ? `<img width="0" height="0" hidden="true" src="${storyDetail.data.image}" referrerpolicy="no-referrer">` : '';
                return cover + utils.ProcessImage(storyDetail.data.body.replace(/<div class="meta">([\s\S]*?)<\/div>/g, '<strong>$1</strong>').replace(/<\/?h2.*?>/g, ''));
            });
