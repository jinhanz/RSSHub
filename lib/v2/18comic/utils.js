        ...
        items = await Promise.all(
            items.map((item) =>
                ctx.cache.tryGet(item.guid, async() => {
                    ...
                    const coverUrl = `https://cdn-msp.18comic.vip/media/albums/${item.link.match(/album\/[0-9]+/)[0].replace("album/", "")}_3x4.jpg`;
                    ...
                    item.author = content('span[data-type="author"]')
                        ...
                        .map((a) => $(a).text())
                        .join(', ');
                    item.description = `<p>${content('#intro-block .p-t-5').text()}</p>` +
                        `<img src="${coverUrl}">` +
                        `<img src="https://split-image.vercel.app/18comic?url=${content('.img_zoom_img img')
                        .toArray()
                        .map((image) => content(image).attr('data-original'))
                        .join('"><img src="https://split-image.vercel.app/18comic?url=')}">`;
