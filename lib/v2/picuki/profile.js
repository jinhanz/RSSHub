...
module.exports = async (ctx) => {
    ...
    const list = $('ul.box-photos [data-s="media"]').get().slice(0,5);
    ...
    async function getMedia(url) {
        ...
        $('.single-photo video').each((_, item) => {
            ...
            if (vidCount==0) {first_cover=videoPoster;}
            vidCount += 1;
            ...
            html += `<br><a href="https://relay-proxy.vercel.app/video/stream?url=` + encodeURIComponent(origin) + '">Open Video Stream</a> | ';
            html += `<a href="${videoPoster}">Poster URL</a><br>`;
        });
        if (imgCount==0 && html.length>0) {
            return html, `<img src=${first_cover}>`
        }
        return html, '';
    }
    ...
    items = [
        ...
        ...(await Promise.all(
            list.map(async (post) => {
                ...
                const [media_displayVideo, first_cover] = await getMedia(postLink);
                const postText = post
                    ...
                    .replace(/[^\S\n]+/g, ' ')
                    .replace(/((?<=\n|^)[^\S\n])|([^\S\n](?=\n|$))/g, '');
                const title = postText.replace(/\.\n/g,'').split(/(<br>|\n)/g)[0].trim().replace(/#.+?(\s|$)/g,'').trim() || 'Untitled';
                let description = art(path.join(__dirname, 'templates/post.art'), {
                    media: displayVideo ? media_displayVideo : deVideo(media_displayVideo),
                    desc: postText,
                    locationLink: post.find('.photo-location .icon-globe-alt a')+first_cover,
                });
