...
module.exports = async (ctx) => {
    ...
    async function getMedia(url) {
        ...
        $('.single-photo img').each((_, item) => {
            item = $(item);
            html += `<image src="${item.attr('src')}"></image>`;
        });
        $('.single-photo video').each((_, item) => {
            item = $(item);
            let videoSrc = undefined;
            if (item.parent().attr('onclick') && item.parent().attr('onclick').match(/https:\/\/.+',/)) {
                videoSrc = item.parent().attr('onclick').match(/https:\/\/.+',/)[0].replace("',", "");
            }
    ...
    let items = await Promise.all(
        list.map(async (post) => {
            ...
            }).replace(/preload="none"/g, `preload="auto" controls="controls"`);
        ...
        if (data && data.stories_container) {
            ...
            const storyItems = $('.item')
                ...
                .map((item) => {
                    ...
                    const href = postBox.attr('data-origin');
