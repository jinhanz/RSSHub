...
module.exports = async (ctx) => {
    ...
    const domain = 'https://www.pixwox.com';
    ...
    async function getMedia(url) {
        ...
        $('.video_img a').each((_, item) => {
            ...
            if (videoPoster) {
                ...
                videoPoster = 'https://media-relay.shaderein.workers.dev/' + videoPoster;
            }
            html += art(path.join(__dirname, 'templates/video.art'), {
                videoSrc,
                videoPoster,
            }).replace('&amp;', '&');
            html += `<br><a href="https://relay-proxy.vercel.app/video/stream?url=` + encodeURIComponent(videoSrc) + '">Open Video Stream</a><br>';
        });
        $('.pic a').each((_, item) => {
