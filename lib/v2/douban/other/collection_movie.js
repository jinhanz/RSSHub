...
module.exports = async(ctx) => {
    ...
    ctx.state.data = {
        ...
        item: await Promise.all(data.map(async({ id, title, info, cover, url, rating, year, release_date, null_rating_reason, description }) => {
            ...
            description = `${info} <br>${release} <br>${rate} <br> <p>${parsed_intro}</p><img src="${cover.url}">`;
            }
            else {
            description = `${info} <br>${release} <br>${rate} <br> <p>${parsed_intro}</p><video width="350" height="240" controls="controls"><source src="${itemInfo.data.trailer.video_url}?autoplay=0"></video><img src="${cover.url}">`;
            }
