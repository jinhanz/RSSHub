            ...
            data.map(async({ id, title, info, card_subtitle, cover, cover_url, url, rating, year, release_date, null_rating_reason, description }) => {
                ...
                if (detailed) {
                    ...
                    if (!itemInfo.data.trailer) {
                        description = `<img src="${cover_url}"><br>${rate} <br>${info} <br>${release} <br> <p>${parsed_intro}</p><br><br>`;
                    } else {
                        description = `<img src="${cover_url}"><br>${rate} <br>${info} <br>${release} <br> <p>${parsed_intro}</p><video width="350" height="240" controls="controls" poster="${itemInfo.data.trailer.cover_url}"><source src="${itemInfo.data.trailer.video_url}?autoplay=0"></video><br><br>`;
                    }
                } else {
                    description = `<img src="${cover_url}"><br>${rate} <br>${info} <br>${release}<br><br>`;
                }
