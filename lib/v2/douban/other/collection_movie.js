        ...
        item: await Promise.all(
            data.map(async ({ id, title, info, cover, url, rating, year, release_date, null_rating_reason, description }) => {
                ...
                if (!itemInfo.data.trailer) {
                    ...
                    description = `<img src="${cover.url}"><br>${info} <br>${release} <br>${rate} <br> <p>${parsed_intro}</p><video width="350" height="240" controls="controls" poster="${itemInfo.data.trailer.cover_url}"><source src="${itemInfo.data.trailer.video_url}?autoplay=0"></video>`;
                }
