        ...
        item: await Promise.all(
            data.map(async({ id, title, info, card_subtitle, cover, cover_url, url, rating, year, release_date, null_rating_reason, description }) => {
                ...
                if (detailed) {
                    ...
                    description = `<img src="${cover_url.replace('.jpg','.webp')}"><br>${rate} <br>${release} <br>${info} <br> <p>${parsed_intro}</p>`;
                    if (itemInfo.data.trailers.length>0) {
                        itemInfo.data.trailers.forEach(trailer => {
                            let video_url = config.hotlink.relay ? `${config.hotlink.relay}/douban/trailer?id=${trailer.id}` : trailer.video_url;
